...
import { saveSubjects, getDailyGoal, saveDailyGoal } from '../storage';
import { formatDuration, SUBJECT_COLORS } from '../utils';
...
export function Subjects({ subjects, onSubjectsChange }: Props) {
  ...
  const [dailyGoal, setDailyGoal] = useState(getDailyGoal().totalMinutes);
    ...
    <div className="subjects-page">
      ...
      <div className="section-card">
        <div className="section-header">
          ...
          {!editingGoal && (
            <button className="btn-text" onClick={() => setEditingGoal(true)}>수정</button>
          )}
        </div>
        {editingGoal ? (
          ...
          <p className="goal-display">{formatDuration(dailyGoal)}</p>
        )}
      </div>
      ...
      <div className="section-card">
        <div className="section-header">
          ...
          {!showForm && (
            <button className="btn-primary btn-sm" onClick={() => setShowForm(true)}>+ 과목 추가</button>
          )}
        </div>
              ...
              <div className="subject-item-left">
                ...
                <div>
                  ...
                  {s.dailyGoalMinutes > 0 && (
                    <div className="subject-item-goal">목표: {formatDuration(s.dailyGoalMinutes)}</div>
                  )}
                </div>
              </div>
