...
export default function App() {
  ...
  const handleNew = () => { setSelected(emptyLog()); setView('edit'); };
        ...
        <main className="app-main">
          ...
          {filtered.length === 0 ? (
            <div className="empty-state">
              ...
              <p>풀이 기록이 없어요.</p>
              <p className="empty-sub">프로그래머스나 백준 문제 풀이를 기록해보세요.</p>
              <button className="btn-primary mt" onClick={handleNew}>+ 첫 일지 작성</button>
            </div>
          ) : (
            <div className="log-grid">
              {filtered.map(log => (
                <div key={log.id} className="log-card" onClick={() => { setSelected(log); setView('view'); }}>
                  ...
                  <div className="log-status" style={{ color: statusOf(log.status).color }}>
                    {statusOf(log.status).label}
                  </div>
                  {log.tags.length > 0 && (
                    <div className="tag-row">
                      {log.tags.slice(0, 4).map(t => <span key={t} className="tag">{t}</span>)}
                      {log.tags.length > 4 && <span className="tag">+{log.tags.length - 4}</span>}
                    </div>
                  )}
                </div>
              ))}
        ...
        <main className="app-main">
          <div className="editor">
            ...
            <div className="editor-actions">
              <button className="btn-primary" onClick={handleSave} disabled={!selected.problemTitle.trim()}>저장</button>
        ...
        <main className="app-main">
          <div className="viewer">
            ...
            {selected.tags.length > 0 && (
              <div className="tag-row section-gap"><label className="section-label">태그</label>{selected.tags.map(t => <span key={t} className="tag">{t}</span>)}</div>
            )}
