...
6. **Progressive enhancement**: 로그인 없이 핵심 기능이 동작해야 하며 계정 기능은 다중 기기 동기화가 실제로 필요할 때만 추가한다.
...
- `가능`: 오늘 해당 유형을 배출할 수 있음
- `예정`: 다음 배출 시간이 아직 시작되지 않음
- `마감`: 오늘 배출 시간이 종료됨
- `불가`: 오늘 배출 대상이 아님
- `확인 필요`: 원본 규칙이 불완전하거나 충돌함
...
MVP의 알림은 사용자가 직접 활성화한 배출일 리마인더만 대상으로 한다.

- 알림은 마케팅 메시지를 포함하지 않는다.
- 브라우저 알림 권한이 거부되면 핵심 기능은 계속 동작한다.
- 마케팅 알림 기능은 MVP 범위에서 제외한다.

### 5.6 Large waste handling
...
## 7. Legal and privacy boundary

이 설계는 정보 제공 서비스 범위를 유지한다.

### 7.1 Waste-management boundary
...
### 7.2 Public data boundary

핵심 일정 데이터는 공공데이터포털의 행정안전부 생활쓰레기배출정보를 우선 사용한다. 이 데이터는 관리구역, 배출방법, 배출요일, 배출시간대, 미수거일, 관리부서 및 연락처 등의 정보를 제공한다.

지자체 웹 페이지를 별도로 크롤링해야 하는 경우 각 출처의 이용조건과 저작권/공공누리 조건을 확인하기 전에는 production 데이터로 편입하지 않는다.

### 7.3 Location and personal data boundary

MVP는 사용자의 실시간 위치를 측위하지 않는다. 사용자가 직접 고른 행정구역만 저장한다.

개인 식별 가능한 상세 주소, GPS 좌표, 이동 이력은 수집하지 않는다.

### 7.4 Notification boundary

배출일 리마인더와 마케팅 메시지를 분리한다. MVP에는 서비스성 배출 리마인더만 포함한다.
...
버리데이는 BloomBouquet 본체에 기능을 직접 추가하지 않는다. 독립적으로 빌드·배포 가능한 프로젝트로 만들고 BloomBouquet에는 제출 URL과 버전을 등록한다.
...
- Vite
- React Router
- TanStack Query only if remote runtime data fetching is needed
- Vitest + Testing Library
- Playwright for critical workflow E2E
...
Official public dataset
  ...
  -> normalizer
  -> validation report
  -> versioned normalized dataset
  -> Beriday web runtime
  -> schedule engine
  -> Today / Weekly / Search UI
```

원본 파싱과 정규화는 사용자 브라우저에서 매번 수행하지 않는다. build/deploy 전에 정규화한 versioned artifact를 사용하거나, 배포 운영상 필요하면 작은 backend endpoint로 제공한다.

### Initial persistence

- `SavedRegion`: LocalStorage
- notification preference: browser-local storage
- normalized public rules: versioned application data
...
알림 권한이 거부되어도 Today/Weekly/Search 기능은 모두 사용할 수 있다.

### Offline

이미 앱에 포함된 normalized dataset과 저장 지역으로 계산 가능한 기능은 계속 동작하도록 설계한다.
...
- normalized dataset은 지역 index를 가져야 한다.
- item search index는 초기 dataset 규모에서 클라이언트 검색을 우선하되 bundle 크기를 측정한다.
- Today 계산은 현재 region의 rule subset만 평가한다.
- production build에서 주요 route의 JavaScript와 dataset 크기를 기록한다.
...
- 외부 링크에는 적절한 `rel` 속성을 사용한다.
- 향후 backend가 생기면 입력 검증, CORS, rate limiting, secrets 분리를 별도 설계한다.

## 18. Data quality and operations

정규화 파이프라인은 import 시 다음 결과를 생성한다.
...
- ambiguous rows
- regions covered
- source updated/imported timestamp

production 배포는 malformed time, unknown weekday, missing region key 등 critical validation error가 남으면 중단한다.

단, 원본이 명시적으로 `정보 없음`인 지역은 앱 자체 build failure로 취급하지 않고 사용자에게 데이터 미제공 상태를 표시한다.
...
10. 이용약관/개인정보처리방침 필요 여부와 실제 수집 항목을 출시 직전에 다시 대조해야 한다.
...
- Frontend: Today/Weekly/Search/Settings implementation
- Backend/API Integration: official data ingestion interface if runtime API becomes necessary
- Database: server persistence is not introduced in MVP unless architecture evidence requires it
...
1. repository bootstrap + PRODUCT/README
2. data contract + fixture-based normalizer
3. schedule engine
4. region selector
5. Today
6. Weekly
7. item disposal dataset/search
8. source/provenance UI
9. notification reminder
10. accessibility/performance/security hardening
11. E2E/QA
12. deployment + BloomBouquet submission

This sequence intentionally proves the data and schedule engine before spending effort on visual polish or account features.
